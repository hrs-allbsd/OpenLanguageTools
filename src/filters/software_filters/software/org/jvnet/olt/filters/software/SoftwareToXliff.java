...
public class SoftwareToXliff {
     ...
     */
    public SoftwareToXliff(String filename, String shortname, String srcLang, String encoding, Logger logger) throws TMCParseException {
        ...
        fileTypeMap.put("dtd", new Integer(MessageListGenerator.MOZDTD_FILE));
        ...
        try {
            this.runConvertInput(filename, shortname,  srcLang, encoding, logger);
    ...
    public static void main(String[] args) {
        try {
            ...
            SoftwareToXliff converter = new SoftwareToXliff(args[0], file.getName(), "en-US", 
                        System.getProperty("file.encoding"), Logger.global);
     ...
     * @return
     */
    public boolean runConvertInput(String filename, String shortname, String sourceLang, String encoding, Logger logger) throws TMCParseException {
        try {
            ...
            return writeXliffFiles(pivFileinfo, sourceLang, filename, shortname, iAlignType);
        } catch (TMCParseException tmcEx) {
    ...
    private boolean writeXliffFiles(TokenCell[] pivFileinfo, String sourceLang,String filename, String shortname, int fileType) {
        ...
        try {
            ...
            SegmenterFormatter formatter =
                    new XliffSegmenterFormatter(format, sourceLang, shortname, xliffWriter, sklWriter, null, wrapper);
            // handy for testing - the rev-formatter creates a dummy translated file
            //SegmenterFormatter formatter = new ReverseXliffSegmenterFormatter(format, sourceLang, "de-DE", shortname, xliffWriter, sklWriter, false, null);
            
            int iNumCells = pivFileinfo.length;
            for (int i = 0; i < iNumCells; i++) {
                /*.println("token["+i+"]="+pivFileinfo[i].getText());
                System.out.println("---------------------");
                    ...
                    case TokenCell.MESSAGE:
                        ...
                        if (pivFileinfo[i].isTranslatable()) {
                            ...
                            if (!pivFileinfo[i].hasTranslation()){
                                ...
                                // for PO files only here atm - PO files may contain
                                // a message string already, so we'd like to
                                // preserve this in the XLIFF file if poss.
                                String translation = pivFileinfo[i].getTranslation();
                                formatter.writeSegment(message, translation, iWordCount);
                            }
    ...
    throws TMCParseException, IOException, Exception {
        ...
        if (!getExtension(filename).equals("dtd")) {
            ...
            // Mozilla DTD files need to be handled specially, because the parser
            // is actually expecting <>& characters in the input, so we can't escape
            // them at this level. They should be escaped later on in the MozDTDFileParserFacade
            // or classes near it.
            reader = readerIn;
        }
    ...
    protected FormatWrapper getFormatWrapper(String type)  {
        ...
        } else if (type.equals("JAVA") || type.equals("PROPERTIES") || type.equals("DTD")){
